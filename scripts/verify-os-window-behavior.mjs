...
const osWindowManager = readFileSync(
  join(root, "src/scripts/os-window-manager.ts"),
  "utf8",
);
...
const bootSplash = readFileSync(
  join(root, "src/components/BootSplash.astro"),
  "utf8",
);
const companyIcon = readFileSync(
  join(root, "src/components/CompanyIcon.astro"),
  "utf8",
);
const experienceSection = readFileSync(
  join(root, "src/components/ExperienceSection.astro"),
  "utf8",
);
const profileIntro = readFileSync(
  join(root, "src/components/ProfileIntro.astro"),
  "utf8",
);
const profilePanels = readFileSync(
  join(root, "src/components/ProfilePanels.astro"),
  "utf8",
);
const socialIcon = readFileSync(
  join(root, "src/components/SocialIcon.astro"),
  "utf8",
);
const writingBrowser = readFileSync(
  join(root, "src/components/WritingBrowser.astro"),
  "utf8",
);
const writingSection = readFileSync(
  join(root, "src/components/WritingSection.astro"),
  "utf8",
);
const baseLayoutScript = readFileSync(
  join(root, "src/scripts/base-layout.js"),
  "utf8",
);
const css = readFileSync(join(root, "src/styles/global.css"), "utf8");
const layout = readFileSync(join(root, "src/layouts/BaseLayout.astro"), "utf8");
const manifest = readFileSync(
  join(root, "public/assets/img/favicon_io/site.webmanifest"),
  "utf8",
);
const persistentStorageToken = "local" + "Storage";
const containsComponent = (source, componentName, attributes) =>
  new RegExp(
    `<${componentName}\\b(?=[\\s\\S]*?${attributes
...
const checks = [
  ...
  },
  {
    ...
    pass:
      ...
      writingSection.includes(
        'import WritingBrowser from "./WritingBrowser.astro"',
      ) &&
  ...
  },
  {
    ...
    pass:
      ...
      !/data-mode-choice="os"[\s\S]{0,180}data-mode-choice="website"/.test(
        index,
      ) &&
      !/data-mode-choice="os"[\s\S]{0,180}data-mode-choice="website"/.test(
        layout,
  ...
  },
  {
    ...
    pass:
      ...
      css.includes(
        'html[data-mode="os"] .website-home {\n    display: revert;',
      ),
  },
  {
  ...
  },
  {
    ...
    pass:
      ...
      baseLayoutScript.includes(
        "`writing&route=${encodeURIComponent(path)}`",
      ) &&
  ...
  },
  {
    ...
    pass:
      /function routeWindowFocus\(\s*id\b[\s\S]*?options\b/.test(index) &&
      /function focusWindow\(\s*id\b[\s\S]*?options\b/.test(index) &&
  ...
  },
  {
    ...
    pass:
      ...
      baseLayoutScript.includes(
        "`writing&article=${encodeURIComponent(path)}&route=${encodeURIComponent(path)}`",
      ) &&
      baseLayoutScript.includes(
        'path === "/blog" || path === "/blog/" || path.startsWith("/archive/")',
      ) &&
      baseLayoutScript.includes(
        "`writing&route=${encodeURIComponent(path)}`",
      ) &&
      ...
      baseLayoutScript.includes(
        "`contact&route=${encodeURIComponent(path)}`",
      ) &&
  ...
  },
  {
    ...
    pass:
      !baseLayoutScript.includes(
        'path.startsWith("/blog/") || path.startsWith("/archive/") ? "writing"',
      ) &&
      baseLayoutScript.includes(
        'path.startsWith("/blog/") && path !== "/blog/"',
      ) &&
  ...
  },
  {
    ...
    pass:
      ...
      index.includes(
        "windowState = closeArticleWindowWithoutRoute(readState())",
      ) &&
      index.includes(
  ...
  },
  {
    ...
    pass: /const navItems = \[\s*\{ href: "\/", label: "About" \},\s*\{ href: "\/blog\/", label: "Writings" \},\s*\{ href: "\/contact\/", label: "Contact" \},?\s*\];/.test(
      layout,
    ),
  },
  {
    ...
    pass:
      /let focusedId(?:: WindowId)? = initialApp \?\? "readme"/.test(index) &&
  ...
  },
  {
    ...
    pass:
      ...
      bootSplash.includes("const buildVersion =") &&
      bootSplash.includes("buildDate.getFullYear()") &&
      bootSplash.includes("pad(buildDate.getMonth() + 1)") &&
      bootSplash.includes("pad(buildDate.getDate())") &&
  ...
  },
  {
    ...
    pass:
      /aria-label="Close Contact\.app"\s*>\s*X\s*<\/button\s*>/.test(index) &&
  ...
  },
  {
    ...
    pass:
      ...
      containsComponent(index, "ExperienceSection", [
...
console.log(
  `OS window behavior verification passed (${checks.length} checks).`,
);
